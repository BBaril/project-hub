  ...
  return (
   ...
   <div className="flex flex-wrap gap-2 mb-2 ml-1">
      {project.tags.map((tag) => (
        <span
         key={tag}
         className={`text-xs px-2 py-1 rounded-full ${tagColor(tag)} text-bbCharcoal cursor-default`}
        >
          {tag}
        </span>
     ))}
    ...
    {/* BUTTONS */}
      <button
        ...
        className="mr-2 text-bbGreen text-sm font-semibold underline hover:text-bbSoftRose"
      ...
      <button
        ...
        className="text-bbBlueGrotto text-sm font-semibold underline hover:text-bbSoftRose">
        UX Journey
      </button>
      )}
      ...
      <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center z-50">
        <div className="bg-white dark:bg-gray-900 p-6 rounded-xl max-w-3xl w-full overflow-auto max-h-[90vh]">
          ...
          <button
            ...
            className="mt-4 px-4 py-2 bg-bbCyanDarker text-bbMidnightIndigo rounded hover:bg-bbSoftRose"
          >
            Close
          </button>
        </div>
    ...
    {/* Screenshot Modal */}
      {showModal && (
        <div className="fixed inset-0 bg-bbCharcoal bg-opacity-50 flex items-center justify-center z-50">
        ...
        {project.demo ? (
          ...
          className="text-bbBlueGrotto underline"
          >
          Live Demo
          </a>
        ) : null}
        ...
        {project.screenshot && (
          <button
            ...
            className="text-bbGreen underline"
          >
        ...
        {project.github && (
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="text-bbOrange underline"
          >
          GitHub
          </a>
       )}
...
function tagColor(tag) {
    ...
    if (frontend.includes(tag)) return 'bg-bbSoftRose';
    if (backend.includes(tag)) return 'bg-bbBlueGrotto';
    if (database.includes(tag)) return 'bg-bbSkyLavender';
    if (gui.includes(tag)) return 'bg-bbCitrine';
    if (mobile.includes(tag)) return 'bg-bbGreen';
    if (creative.includes(tag)) return 'bg-bbCyanDarker';
